...
# * Sep 3 (Suganya & VTJ)
# * - Added global param and multi-purpose CIBC Specific
...
class CdocmWlConfig:
  ...
  def getParamField( self ):
    connection = self.xmldoc.getElementsByTagName("param")
    ...
    for prop in connectProp:
      ...
      if propName == 'db-user':
        self.guser = propValue
      if propName == 'db-password':
        self.gpassword = propValue
      if propName == 'url':
        self.gurl = propValue
      if propName == 'filestore':
        self.gfilepath = propValue
      if propName == 'filestore2':
        self.gfilepath2 = propValue  
  ...
  def setTarget( self ):
     ...
     for prop in connectProp:
       ...
       if 'value' in prop.attributes.keys():
         propValue = prop.attributes['value'].value
       if propName == 'target':
         self.gtargetType = prop.attributes['type'].value	
         self.gtarget = propValue		
  ...
  def updateJMSModule(self, mbean, properties, name):
    for _property in properties:
      ...
      if propName == "target":
        ...
        self.setTarget()
        targetMbeanPath = '/' + self.gtargetType + '/' + self.gtarget
  ...
  def updateJDBCDatasource(self, mbean, properties, name):
    for property in properties:
	 ...
	 #self.getParamField()
        driverparamsMbean.setUrl(self.gurl)
      ...
      if propName == 'Password':
        jdbcMbean = mbean.getJDBCResource()
        driverparamsMbean = jdbcMbean.getJDBCDriverParams()
        driverparamsMbean.setPassword(self.gpassword)
      if propName == 'User':
        jdbcMbean = mbean.getJDBCResource()
        driverparamsMbean = jdbcMbean.getJDBCDriverParams()
  ...
  def createConnectionFactory(self, name, properties):
    ...
    if self.pathExists(mbeanPath):
      mbean = getMBean(mbeanPath)
      self.updateConnectionFactory(mbean,properties, name)
    else:
      # Create the resource
      jmsPath = '/JMSSystemResources/'+jmsModule
  ...
  def updateConnectionFactory(self, mbean, properties, name):
    for property in properties:
      propName = property.attributes['name'].value
      if 'value' in property.attributes.keys():
        propValue = property.attributes['value'].value
  ...
  def updateFileStore(self, mbean, properties, name):
    for property in properties:
      ...
      if 'value' in property.attributes.keys():
        #propValue = property.attributes['value'].value
        propValue = self.gfilepath
        print propValue
      if self.gfilepath == ' ':
        #propValue = property.attributes['value'].value
        propValue = self.gfilepath2
      self.gfilepath = ' '  
      if 'secret' in property.attributes.keys():
        propValue = self.secret[name][propName]
  ...
  def createUniformDistributedQueue(self, name, properties):
    p("Creating UniformDistributedQueue", name)
    self.wl_update()
    jmsModule = ''
    for property in properties:
      propName = property.attributes['name'].value
  ...
  def createUniformDistributedTopic(self, name, properties):
    p("Creating UniformDistributedTopic", name)
    self.wl_update()
    jmsModule = ''
    for property in properties:
      propName = property.attributes['name'].value
  ...
  def updateJMSServer(self, mbean, properties, name):
    for property in properties:
      ...
      if propName == 'Datastore':
        ...
        cd('/')
      if propName == 'target':
        #argetType = property.attributes['type'].value
        #targetMbeanPath = '/' + targetType + '/' + propValue
	self.setTarget()
	targetType = self.gtargetType
	target = self.gtarget
        ...
        if targetMbean is None:
          ...
          if targetType == 'Clusters':
            targetType = 'Cluster'
          target = jarray.array([ObjectName('com.bea:Name='+target+',Type='+targetType)], ObjectName)
          set('Targets', target)
          cd('/')
          # mbean.addTarget(targetMbean)
